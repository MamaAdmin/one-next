...
export default function SprintJoin() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (!userData.user) {
        // Save token, then send to auth
        sessionStorage.setItem("pending_sprint_invite", token);
        navigate("/auth?redirect=" + encodeURIComponent(`/sprint/join/${token}`));
        return;
      }
      const { data, error } = await supabase.rpc("redeem_sprint_invitation" as never, {
        p_token: token,
      } as never);
      if (cancelled) return;
      if (error) {
      ...
      const sprintId = data as unknown as string;
